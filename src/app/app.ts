...
export class App implements OnDestroy {
  ...
  readonly availableSkins = AVAILABLE_SKINS;
   ...
   */
  /**
   * Canonical app URL used in share text, invite links and the exit-interview redirect.
   * Order of precedence:
   *   1) `window.location.origin` (always wins on the client — covers any deploy URL)
   *   2) `process.env['APP_URL']` (lets a deployment override during SSR rendering)
   *   3) Firebase Hosting fallback (the historical canonical URL)
   */
  appUrl(): string {
  ...
  copyCompanyInvite() {
    ...
    if (!c) return;
    const url = this.appUrl();
    const text = `🏢 Join ${c.name} on LinkedOut.\nJoin code: ${c.joinCode}\n${url}`;
  ...
  readonly AVAILABLE_MODES = AVAILABLE_MODES;
  ...
  private buildExitInterview(snap: {
    ...
    const url =
      (this.appUrl()) + "?utm_source=exit_interview";
  ...
  generateLinkedInPost() {
    ...
    this.linkedInPost = `🚀 Thrilled to announce my transitioning phase! Played LinkedOut${modeText} and reached [${title}] with ${syn} Synergy. I boldly 'synergized' ${this.emailsSynergized} Urgent Emails and fired ${this.doersFired} actual doers to protect company culture.${statsStr}\n\nCan you climb the corporate ladder faster? 📈💼 #ThoughtLeader #Grindset #CorporateRun`;

    this.slackPost = `:chart_with_upwards_trend: *LinkedOut${modeText}*
*Title:* ${title}
...
\nI challenge you to beat my performance metrics directly. See you at the top. :handshake:`;
  ...
  readonly AVAILABLE_AVATARS = AVAILABLE_AVATARS;
  ...
  async shareNative() {
    ...
    const title = this.currentTitle();
    const url = this.appUrl();
    const text = `📈 I just climbed to ${title.toUpperCase()} with ${score} synergy on LinkedOut. Think you can beat me?`;
    try {
      const { Capacitor } = await import('@capacitor/core');
      if (Capacitor.isNativePlatform()) {
        const { Share } = await import('@capacitor/share');
        await Share.share({ title: 'LinkedOut', text, url, dialogTitle: 'Share your humiliation' });
      } else if (typeof navigator !== 'undefined' && (navigator as Navigator & { share?: (data: ShareData) => Promise<void> }).share) {
        await (navigator as Navigator & { share: (data: ShareData) => Promise<void> }).share({ title: 'LinkedOut', text, url });
      } else {
  ...
  private CAPTIONS: Record<string, string> = {
    'instagram-story': "📈 I just made it to {{title}} on LinkedOut. Tap to play.",
    ...
    'native':          "I climbed to {{title}} with {{score}} synergy on LinkedOut. Beat me? {{url}}",
  };
